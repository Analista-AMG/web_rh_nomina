        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-xs text-gray-500 dark:text-gray-400 uppercase border-b border-light-border dark:border-dark-border bg-gray-50 dark:bg-[#1b2431]">
                    ...
                    <th class="px-5 py-3 text-center font-medium">Origen → Destino</th>
            ...
            <tbody class="divide-y divide-light-border dark:divide-dark-border">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-[#1b2431] transition-colors">
                    ...
                    <td class="px-5 py-3 text-center text-gray-600 dark:text-gray-300 text-xs whitespace-nowrap">
                        @php $dias = $p->fecha_inicio->diffInDays($p->fecha_fin) + 1; @endphp
                        <div>{{ $p->fecha_inicio->format('d/m/Y') }} → {{ $p->fecha_fin->format('d/m/Y') }}</div>
                    ...
                    <td class="px-5 py-3 text-center">
                        ...
                        @elseif($p->estado === 'rechazado')
                            ...
                            @endif
                        @elseif($p->estado === 'anulado')
                            <span class="inline-flex items-center gap-1 text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-2 py-1 rounded-full">
                                <i class="fa-solid fa-rotate-left"></i> Anulado
                            </span>
                            @if($p->motivo_rechazo)
                            <p class="text-[11px] text-gray-400 mt-1 max-w-[140px] mx-auto truncate" title="{{ $p->motivo_rechazo }}">
                                {{ $p->motivo_rechazo }}
                            </p>
                            @endif
                        @else
                    ...
                    <td class="px-5 py-3 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            {{-- Anular (aprobadores, en activos, próximos e historial con estado aprobado) --}}
                            @if($puedeAprobar && in_array($modo, ['activos', 'proximos']) || ($modo === 'historial' && $puedeAprobar && $p->estado === 'aprobado'))
                            <button type="button"
                                onclick="abrirModalAnular({{ $p->id }})"
                                class="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-colors cursor-pointer"
