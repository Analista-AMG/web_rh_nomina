...
class ContratoAlertasController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ── Filtros económicos — vía movimientos (cubre activos y finalizados) ─
        $applyFiltros = fn($q) => $q
            ->when($empresa,       fn($q) => $q->whereHas('movimientos.planilla', fn($p) => $p->where('nombre_empresa', $empresa)))
            ->when($planillaId,    fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('planilla_id', $planillaId)))
            ->when($centroCostoId, fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('centro_costo_id', $centroCostoId)))
            ->when($familiaId,     fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('familia_id', $familiaId)))
        ...
        $opts = fn(array $skip = []) => $applyTab(Contrato::query())
            ->when(!in_array('empresa', $skip)  && $empresa,       fn($q) => $q->whereHas('movimientos.planilla', fn($p) => $p->where('nombre_empresa', $empresa)))
            ->when(!in_array('planilla', $skip) && $planillaId,    fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('planilla_id', $planillaId)))
            ->when(!in_array('centro', $skip)   && $centroCostoId, fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('centro_costo_id', $centroCostoId)))
            ->when(!in_array('familia', $skip)  && $familiaId,     fn($q) => $q->whereHas('movimientos', fn($m) => $m->where('familia_id', $familiaId)))
        ...
        // ── Dropdowns dinámicos (IDs desde último movimiento disponible) ───────
        $planillaIdsEmpresa = $opts(['empresa'])
            ->with('movimientoActual:id,contrato_id,planilla_id', 'movimientos:id,contrato_id,planilla_id')
            ->get()
            ->map(fn($c) => $c->movimientoActual?->planilla_id
                ?? $c->movimientos->sortByDesc('inicio')->first()?->planilla_id)
            ->filter()->unique();
        ...
        $planillaIdsPlanilla = $opts(['planilla'])
            ->with('movimientoActual:id,contrato_id,planilla_id', 'movimientos:id,contrato_id,planilla_id')
            ->get()
            ->map(fn($c) => $c->movimientoActual?->planilla_id
                ?? $c->movimientos->sortByDesc('inicio')->first()?->planilla_id)
        ...
        $centroCostoIds = $opts(['centro'])
            ->with('movimientoActual:id,contrato_id,centro_costo_id', 'movimientos:id,contrato_id,centro_costo_id')
            ->get()
            ->map(fn($c) => $c->movimientoActual?->centro_costo_id
                ?? $c->movimientos->sortByDesc('inicio')->first()?->centro_costo_id)
            ->filter()->unique();

        $centrosCosto = CentroCosto::whereIn('id', $centroCostoIds)
            ->select('id', 'nombre_centro_costo')->orderBy('nombre_centro_costo')->get();

        $familiaIds = $opts(['familia'])
            ->with('movimientoActual:id,contrato_id,familia_id', 'movimientos:id,contrato_id,familia_id')
            ->get()
            ->map(fn($c) => $c->movimientoActual?->familia_id
                ?? $c->movimientos->sortByDesc('inicio')->first()?->familia_id)
            ->filter()->unique();
    ...
    public function exportar(Request $request)
    {
        ...
        foreach ($registros as $c) {
            $persona  = $c->persona;
            $mov      = $c->movimientoActual ?? $c->movimientos->sortByDesc('inicio')->first();
            ...
            $nombreCol = trim(($persona->apellido_paterno ?? '') . ' ' . ($persona->apellido_materno ?? '') . ', ' . ($persona->nombres ?? ''));
